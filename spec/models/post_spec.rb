...
RSpec.describe Post, type: :model do
  ...
  context 'methods' do
    it 'updates user post_counter after create' do
      ...
      post = Post.create(author: user, title: 'My Post', comment_counter: 0, likes_counter: 0)
      user.reload
    ...
    it 'returns the most recent comments, limited to 5' do
      ...
      newest_comment = post.comments.create(user: user, text: 'This is the newest comment', created_at: Time.now)
      middle_comment1 = post.comments.create(user: user, text: 'This is a middle comment 1', created_at: 1.day.ago)
      middle_comment2 = post.comments.create(user: user, text: 'This is a middle comment 2', created_at: 2.days.ago)
      middle_comment3 = post.comments.create(user: user, text: 'This is a middle comment 3', created_at: 3.days.ago)
      older_comment = post.comments.create(user: user, text: 'This is an older comment', created_at: 4.days.ago)

      return_most_recent_comments = post.return_most_recent_comments

      expect(return_most_recent_comments).to eq([newest_comment, middle_comment1, middle_comment2, middle_comment3, older_comment])
    end
  end
end
